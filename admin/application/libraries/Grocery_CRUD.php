...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function get_string_input($field_info,$value)
	{
		...
		$input = "<input id='field-{$field_info->name}' class='lk form-control' name='{$field_info->name}' type='text' value=\"$value\" $extra_attributes />";
		return $input;
	}
	...
	protected function get_relation_input($field_info,$value)
	{
		...
		$select_title = str_replace('{field_display_as}',$field_info->display_as,$this->l('set_relation_title'));
		$input = "<select id='field-{$field_info->name}'  name='{$field_info->name}' class='$ajax_or_not_class' data-placeholder='$select_title' style='width:510px'>";
